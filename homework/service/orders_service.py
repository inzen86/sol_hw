...
class OrderService:
    ...
    def get_products_if_exists(self, order_id):
        order = self.get(order_id)
        if not order:
            return
        return self.get_products(order_id)
    ...
    def update_status(self, order_id, new_status):
        ...
        order = Order.from_row(order_row)
        if order.status != 'NEW' or new_status != 'PAID' or new_status is None:
    ...
    def validate_product_ids(self, product_ids) -> list:
        ...
        for product_id in product_ids:
            if product_id in validated_ids:
                return []
    ...
    def replace_product(self, order_id, product_uuid, replacement_id, replacement_quantity):
        ...
        replaceable_item_in_order = False
        for product in order.products:
            if product_uuid == product.id:
                replaceable_item_in_order = True
                break
        if not replaceable_item_in_order:
            return 'Not found', 404
